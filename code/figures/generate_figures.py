...
def compute_kmer_frequencies(seq, k=3):
...
def plot_diversity_overall(df):
    ...
    summary['Model'] = summary['Model'].replace({'GRPO': 'RL', 'SFT+GRPO': 'SFT+RL'})
...
def plot_completion_benchmark(df):
    ...
    completion['Model'] = completion['Model'].replace({'SFT_GRPO': 'SFT+RL'})
...
def plot_surprisal_benchmark():
    ...
    surprisal['Model'] = surprisal['Model'].replace({'SFT_GRPO': 'SFT+RL'})
...
def plot_ncbi_diversity():
    """Plot NCBI BLAST diversity results."""
    fig, ax = plt.subplots(figsize=(8, 6))
    ...
    ax.set_xlabel('Model', fontsize=12)
    ax.set_ylabel('NCBI BLAST Diversity Ratio', fontsize=12)
    ax.set_title('Uniqueness of Generated Sequences (NCBI BLAST)', fontsize=14, fontweight='bold')
    ax.set_ylim(0, 1.1)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)

    plt.tight_layout()
    plt.savefig(PUB_DIR / 'fig10_ncbi_diversity.png', dpi=300, bbox_inches='tight')
    plt.savefig(PUB_DIR / 'fig10_ncbi_diversity.pdf', bbox_inches='tight')
    plt.close()
    print("Saved: fig10_ncbi_diversity")
...
def plot_completion_by_plasmid():
    ...
    plt.tight_layout()
    plt.savefig(PUB_DIR / 'fig11_completion_by_plasmid.png', dpi=300, bbox_inches='tight')
    plt.savefig(PUB_DIR / 'fig11_completion_by_plasmid.pdf', bbox_inches='tight')
    plt.close()
    print("Saved: fig11_completion_by_plasmid")
...
def plot_ori_amr_counts(df):
    ...
    plt.tight_layout()
    plt.savefig(PUB_DIR / 'fig12_ori_amr_counts.png', dpi=300, bbox_inches='tight')
    plt.savefig(PUB_DIR / 'fig12_ori_amr_counts.pdf', bbox_inches='tight')
    plt.close()
    print("Saved: fig12_ori_amr_counts")
