...
introduce Plasmid-RL, a reinforcement learning framework for DNA language
models that generates structurally valid and functionally coherent
plasmid sequences. Using Group Relative Policy Optimization with a
reward function based on synthetic biological constraints, our model achieves 97\% quality
control pass rate compared to 6\% for the pretrained baseline.
Remarkably, beyond explicitly optimized features, the model exhibits
emergent biological parallels: generated sequences match natural plasmids
in thermodynamic stability, codon usage patterns, and ORF length
distributions, properties not directly encoded in the reward function.
These results suggest that RL training steers the model toward
biologically coherent regions of sequence space, analogous to how
evolutionary optimization produces conserved functional patterns across
diverse organisms.
\end{abstract}
...
These capabilties have vastly increased the utility of language models 
...
identical outputs. This is consistent with RL optimization finding
...
for the base model. The SFT model generated 100\% novel sequences.
...
and other applications. See full details in the appendix. The distribution 
of the generated samples to each other and the real samples. We calculate
...
post-trained model (yellow) closely matches real plasmid distributions
...
energy), while the base model (green) and SFT model (orange) show
...
Dispite generally making up a smaller percentage of the whole sequence, GC
content is partally encoded by the reward function, as regions with higher GC content are more likely to be rewarded.
...
factored in at all excluding the weakly corrolated repeat penatly 
...
with the standard deviation shrinking substantially from 6.144 to 2.977.
This improvement is unexpected, as
...
less, leading to  the shrunken variance and a more similar mean to the real
...
sequence is missing from the library either by ommission or because it has
...
\subsection{Emergent Properties and Evolutionary Analogies}

The emergence of biologically realistic properties, reflexing those of real
plasmids, while explicitly optimized mirros an evolutionary processes in that 
produce complex, correlated traits as byproducts of selecting for primary
 fitness criteria. Just as evolution optimizes for survival
and reproduction while producing emergent phenomena like protein folding
patterns and metabolic network architecture, our RL process optimizes for
functional annotations while inducing realistic thermodynamic and
sequence composition properties. This suggests that appropriately
structured reward functions may capture sufficient biological constraints
to guide models toward broadly realistic sequence space regions.
...
conditioning will promote more diveristy of sample and make evaluation of 
the model's performance on the task of plasmid design much more understable.
...
Future work will extend this framework to conditional generation,
enabling user-directed plasmid design (e.g., ``express protein X in E.
coli with high copy number''), at which point experimental validation of
model-generated sequences will be critical. Additionally, we aim to apply
these techniques to other models and biological sequence classes, further
demonstrating the generality of RL post-training for biomolecular design.
...
comparison and benchmarking analyses in Section 4.2:

\begin{small}
\begin{tabular}{llp{5.5cm}}
\toprule
Plasmid & Size (bp) & Description \\
\midrule
pUC19 \cite{puc19} & 2,686 & High-copy cloning vector \\
pBluescript \cite{pbluescript} & 2,961 & Cloning vector \\
pBR322 \cite{pbr322} & 4,361 & Classic cloning vector \\
pACYC184 \cite{pacyc184} & 4,245 & Low-copy vector \\
pBAD24 \cite{pbad24} & 4,542 & Arabinose-inducible expression \\
pEGFP \cite{pegfp} & 4,733 & GFP expression vector \\
pGEX-4T-1 \cite{pgex} & 4,969 & GST fusion expression \\
pET-28a \cite{pet28a} & 5,369 & T7 expression vector \\
pcDNA3 \cite{pcdna3} & 5,428 & Mammalian expression \\
px330 \cite{px330} & 8,074 & CRISPR/Cas9 vector \\
\bottomrule
\end{tabular}
\end{small}

These plasmids represent widely-used vectors spanning diverse applications
(cloning, protein expression, mammalian transfection, genome editing) and
size ranges (2.7--8.1 kb), providing a representative benchmark for
evaluating the biological realism of generated sequences.
