...
pass rates and greater sequence novelty than baseline methods,
...
thresholds:
\begin{itemize}
    \item \textbf{Exists:} identity $\geq$ 99\% and coverage $\geq$ 95\%
    \item \textbf{Similar:} identity $\geq$ 95\% and coverage $\geq$ 80\%
    \item \textbf{Novel:} anything outside the above criteria
\end{itemize}
...
Both supervised fine-tuning and reinforcement learning substantially
increase the probability of generating plasmids that pass our
bioinformatics QC pipeline, with RL showing the largest improvement.
Table~\ref{tab:qc_results} summarizes pass rates across models and
prompts.

\begin{table}[t]
\caption{Quality control pass rates across model variants and prompt
types. RL achieves dramatically higher validity while maintaining
diversity.}
...
\begin{tabular}{lcccc}
\toprule
Prompt Type & Base & SFT & RL & SFT + RL\\
\midrule
Weak (ATG)  & 0\%  & 16\% & 98\%  & 94\%\\
Strong (GFP) & 12\% & 6\%  & 96\%  &  96\% \\
\midrule
\textbf{Overall} & \textbf{6\%} & \textbf{11\%} & \textbf{97\%}  & \textbf{95\%}\\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
...
rises from 6\% with the base model to 13\% with the SFT model and
finally to 81\% with the RL model, representing more than an
...
proportion classified as novel remains substantial for the all models.
Using our novelty thresholds, the RL model produces approximately 90\%
novel plasmids among its QC-passing samples, compared to approximately
18\% for the base model.  The SFT model always generated novel sequences
when tested with these prompts When normalized over all 100 rollouts per
model, the RL model produces 52\% sequences that are both QC-valid and
novel, compared to 7\% for SFT and 6\% for the base model.
...
\vskip 0.2in
\begin{center}
\includegraphics[width=\textwidth]{fig_distribution_grid_combined.png}
\caption{Distribution comparison between generated and real plasmids.
The RL model (green) closely matches real plasmid distributions (orange)
across multiple metrics not explicitly encoded in the reward function.}
...
baselines. Our model generates structurally valid plasmids at an 81\%
rate (compared to 6\% for the base model) while maintaining 60\% novelty
